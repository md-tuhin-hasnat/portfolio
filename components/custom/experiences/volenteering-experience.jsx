...
export function VolenteeringExperience() {
  ...
  useEffect(() => {
    const fetchExperiences = async () => {
      try {
        const query = `*[_type == "experience" && type == "volunteer"] | order(duration desc)`
        const data = await client.fetch(query)
        setExperiences(data)
      } catch (error) {
        console.error('Error fetching volunteering experiences:', error)
      } finally {
        setLoading(false)
      }
    }
